...
class InitUserInfoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnContinue.setOnClickListener {
            ...
            when {
                TextUtils.isEmpty(weight) -> Snackbar.make(it, "Пожалуйста, введите свой вес", Snackbar.LENGTH_SHORT).show()
                ...
                TextUtils.isEmpty(workTime) -> Snackbar.make(
                    it,
                    "Пожалуйста, введите ежедневное время тренировки в минутах",
                    Snackbar.LENGTH_SHORT
                ).show()
                workTime.toInt() > 500 || workTime.toInt() < 0 -> Snackbar.make(
                    it,
                    "Пожалуйста, введите действительное ежедневное время тренировки в мин",
                    Snackbar.LENGTH_SHORT
                ).show()
                else -> {
    ...
    override fun onBackPressed() {
        ...
        Snackbar.make(
            ...
            "Пожалуйста, нажмите НАЗАД снова чтобы выйти",
            Snackbar.LENGTH_SHORT
        ).show()
